...
export const customerService = {
  list: (): Promise<Customer[]> =>
    ...
    delay([...mockDb.customers]),

  create: (data: { name: string; phone?: string; address?: string }): Promise<Customer> => {
    // return apiClient.post<Customer>('/api/customers', data).then((r) => r.data);
  ...
  update: (id: string, data: { name?: string; phone?: string; address?: string }): Promise<Customer> => {
    // return apiClient.put<Customer>(`/api/customers/${id}`, data).then((r) => r.data);
  ...
  deactivate: (id: string): Promise<void> => {
    // return apiClient.delete(`/api/customers/${id}`).then((r) => r.data);
  ...
  getPricing: (id: string): Promise<CustomerPricingItem[]> => {
    // return apiClient.get<CustomerPricingItem[]>(`/api/customers/${id}/pricing`).then((r) => r.data);
  ...
  updatePricing: (id: string, items: { product_id: string; custom_price?: number }[]): Promise<void> => {
    // return apiClient.put(`/api/customers/${id}/pricing`, { items }).then((r) => r.data);
  ...
  getDebt: (id: string): Promise<{ outstanding_debt: number }> => {
    // return apiClient.get<{ outstanding_debt: number }>(`/api/customers/${id}/debt`).then((r) => r.data);
    const c = mockDb.customers.find((c) => c.id === id);
    return delay({ outstanding_debt: c?.outstanding_debt ?? 0 });
  },

  getContainerLoans: (id: string) => {
    // return apiClient.get(`/api/customers/${id}/container-loans`).then((r) => r.data);
